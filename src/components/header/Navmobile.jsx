        ...
        <div className="lg:hidden relative">
            <nav className={`fixed top-0 w-full z-50 bg-white h-14 flex justify-between items-center px-6 ${open ? "" : "shadow"}`}>
                <div className="flex items-center space-x-2">
                    ...
                    <Link to="/" className="text-xl md:text-2xl font-bold">EduPress</Link>
                </div>
                ...
                <ul className="flex flex-col w-full p-4 space-y-4 border-2 border-orange-500 rounded-md">
                    {navItems.map((item, idx) => {
                        ...
                        return (
                            <li key={idx}>
                                {item.to
                                    ? <Wrapper smooth={item.hash} to={item.to} className={`block hover:text-orange-500 ${isActive ? "text-orange-500" : ""}`}>{item.text}</Wrapper>
                                    : <span className="block hover:text-orange-500 cursor-pointer">{item.text}</span>}
                            </li>
                        );
                    })}
                </ul>
            ...
            <div className={`fixed inset-0 z-30 backdrop-blur-[1px] bg-black transition-opacity duration-300 ${open ? "opacity-40 pointer-events-auto" : "opacity-0 pointer-events-none"}`} onClick={() => setOpen(false)} />
        </div>
    );
};
